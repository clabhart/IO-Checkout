...
namespace IOCheckoutTool
{
    ...
    {
        #region Fields
        internal static DirectoryInfo _projectsFolder = new(Path.Combine(Environment.ExpandEnvironmentVariables("%userprofile%"), "Documents", "IO Checkout Files"));

        private static readonly string[] _fbms =
        {
        ...
        #region Properties
        internal XmlDocument CPFile { get; set; }
        internal ErrorHandler Handler { get; set; }
        internal DirectoryInfo Project { get; set; }
        internal DirectoryInfo SaveLocation { get; set; }
        ...
        private void BuildCP_Click(object sender, EventArgs e)
        {
            Cursor = Cursors.WaitCursor;
            Save();
            string savepath = Path.Combine(Environment.ExpandEnvironmentVariables("%userprofile%"), "Documents", "IO Checkout Files", ProjectName);
            List<TreeNode> fbms = DataBase.SelectedNode.Nodes.Cast<TreeNode>().Where(n => n.Name == "FBM").ToList();
            if (Directory.Exists(savepath))
            {
                SaveLocation = new(savepath);
            }
            else
            {
                SaveLocation = Directory.CreateDirectory(savepath);
        ...
        private void DeleteProject(object sender, EventArgs e)
        {
            ...
            Directory.Delete(Path.Combine(_projectsFolder.FullName, menuitem.Name), true);
            LoadProjects(_projectsFolder.FullName);
            LoadDeletes(_projectsFolder.FullName);
        }
        ...
        private void IOCheckout_Load(object sender, EventArgs e)
        {
            ...
            if (!Directory.Exists(_projectsFolder.FullName))
            {
                Directory.CreateDirectory(_projectsFolder.FullName);
            }
            CurrentProject.Text = string.Empty;
            LoadFBMs();
            LoadProjects(_projectsFolder.FullName);
        ...
        private void LoadDatabase(out bool loaded)
        {
            Cursor = Cursors.WaitCursor;
            DataBase.Nodes.Clear();
            StreamReader reader = new(Path.Combine(Project.FullName, "CPs.txt"));
            TreeNode tree = DataBase.Nodes.Add("Database", ProjectName);
            tree.ImageIndex = 1;
            tree.SelectedImageIndex = 1;
            tree.StateImageIndex = 1;
            string path = Path.Combine(Project.FullName, $"{ProjectName}.xml");
            while (!reader.EndOfStream)
            {
        ...
        private void LoadProject(object sender, EventArgs e)
        {
            ...
            Cursor = Cursors.WaitCursor;
            ToolStripMenuItem menuitem = (ToolStripMenuItem)sender;
            ProjectName = menuitem.Name;
            Project = new(Path.Combine($"{_projectsFolder.FullName}\\{ProjectName}\\Project"));
            string errorpath = Path.Combine(_projectsFolder.FullName, ProjectName, "Logs");
            DirectoryInfo info;
        ...
        {
            Cursor = Cursors.WaitCursor;
            NewProject project = new(_projectsFolder) { StartPosition = FormStartPosition.CenterParent };
            project.ShowDialog();
        ...
        {
            string filename = Path.Combine(Project.FullName, $"{Project.Name}.xml");
        ...
        private void Save()
        {
            ...
            CPFile.Save(Path.Combine(Project.FullName, $"{ProjectName}.xml"));
        }
